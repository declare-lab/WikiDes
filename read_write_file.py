...
def write_single_dict_to_json_file(out_file_name, data_dict, try_no = 0, file_access = 'a', format_json = False):
    ...
    try:        
        data_string = json.dumps(data_dict, indent=4, separators=(', ', ': '), ensure_ascii=False, cls=NumpyEncoder)
        with open(out_file_name, file_access, encoding='utf-8') as outfile:
            if (format_json == False):
                outfile.write(data_string + ',\n')
            else:
...
def load_list_from_json_file(out_file_name, format_json = True, try_no = 0):
    """
    ...
    try:
        ...
        print('Error -- load_list_from_json_file: ', e)
        try_no += 1
        if (try_no <= 10):
            custom_time.sleep(2)
            return load_list_from_json_file(out_file_name, format_json, try_no) # try to read file again
        pass
    
    return result_list
    

def load_list_from_jsonl_file(out_file_name, try_no = 0):
    """
        load list from *.json file
            out_file_name: string - file name
            return: list - a return list
    ...
    try:
        ...
        if (try_no <= 10):
            custom_time.sleep(2)
            return load_list_from_jsonl_file(out_file_name, try_no) # try to read file again
        pass
...
def read_list_from_tsv_file(file_name):
    """
        write a text in a new line to file
            file_name: string - file name
            return: list
    ...
    try:
        ...
        print('Error -- read_list_from_tsv_file: ', e)
        with open(file_name, 'a', encoding='utf-8') as f: # create a new empty file
            f.close()
